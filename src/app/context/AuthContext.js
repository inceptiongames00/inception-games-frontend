...
export function AuthProvider({ children }) {
  ...
  const loginVerifyOTP = useCallback(
    async (email, otp) => {
      ...
      if (!accessToken && data.authentication) {
        ...
        refreshToken =
          data.authentication.refreshToken || data.authentication.refresh_token;
      }
      ...
      const userObj = {
        ...
        avatar:
          userData?.avatar_url || userData?.avatar || userData?.avatarUrl || "",
        banner:
          userData?.banner_url || userData?.banner || userData?.bannerUrl || "",
        discord: userData?.discord || userData?.discordId || "",
        bio: userData?.bio || userData?.biography || "",
        primaryGame:
          userData?.primary_game ||
          userData?.primaryGame ||
          userData?.game ||
          "",
        gameRole:
          userData?.game_role || userData?.gameRole || userData?.role || "",
   ...
   */
  const registerPersonalInfo = useCallback(
    async (email, fullName, username, discord = "", bio = "") => {
      setError(null);
      ...
      if (!res.ok) {
        const msg =
          data.error || data.message || "Failed to save personal info";
        setError(msg);
        throw new Error(msg);
      }
      ...
      if (typeof window !== "undefined") {
        sessionStorage.setItem(
          "sns_registration_info",
          JSON.stringify({ email, fullName, username, discord, bio }),
        );
      }
   ...
   */
  const registerProfileImages = useCallback(
    async (email) => {
      setError(null);
      ...
      const userId =
        typeof window !== "undefined"
          ? sessionStorage.getItem("temp_userId")
          : null;
    ...
    async (userId, formDataToSend) => {
      ...
      try {
        ...
        const updatedUser = {
          ...
          id: userId,
          fullName:
            formDataToSend.get("full_name") ||
            apiUserData.full_name ||
            currentUser.fullName ||
            "",
          username:
            formDataToSend.get("username") ||
            apiUserData.username ||
            currentUser.username ||
            "",
          phone:
            formDataToSend.get("phone") ||
            apiUserData.phone ||
            currentUser.phone ||
            "",
          bio:
            formDataToSend.get("bio") ||
            apiUserData.bio ||
            currentUser.bio ||
            "",
          discord:
            formDataToSend.get("discord") ||
            apiUserData.discord ||
            currentUser.discord ||
            "",
          primaryGame:
            formDataToSend.get("primary_game") ||
            apiUserData.primary_game ||
            currentUser.primaryGame ||
            "",
          gameRole:
            formDataToSend.get("game_role") ||
            apiUserData.game_role ||
            currentUser.gameRole ||
            "",
          rank:
            formDataToSend.get("rank") ||
            apiUserData.rank ||
            currentUser.rank ||
            "",
          continent:
            formDataToSend.get("continent") ||
            apiUserData.continent ||
            currentUser.continent ||
            "",
          country:
            ...
            "",
          avatar:
            apiUserData.avatar_url ||
            apiUserData.avatar ||
            currentUser.avatar ||
            "",
          banner:
            apiUserData.banner_url ||
            apiUserData.banner ||
            currentUser.banner ||
            "",
        };
