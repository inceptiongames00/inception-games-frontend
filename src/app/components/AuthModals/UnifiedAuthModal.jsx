...
export default function UnifiedAuthModal({
  ...
  const handleRegisterStep4GamingProfile = async (e) => {
    ...
    try {
      setIsLoading(true);
      const region = [selectedCity, selectedCountry, selectedContinent]
        .filter(Boolean)
        .join(", ");
        ...
        <>
          <motion.div variants={itemVariants} className="relative py-2">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-gradient-to-b from-[#171717] to-[#0a0a14] text-gray-500">
              ...
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    disabled={isLoading}
                    className="w-full pl-10 pr-4 py-2.5 bg-white/[0.03] border border-white/[0.08] rounded-xl text-white focus:outline-none focus:border-purple-500/50 focus:bg-white/[0.05] transition text-sm appearance-none cursor-pointer"
                  >
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {ROLES[formData.game].map((r) => (
                        <button
                          ...
                          disabled={isLoading}
                          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${formData.role === r ? "bg-purple-500/20 text-purple-300 border border-purple-500/40" : "bg-white/[0.03] text-gray-500 border border-white/[0.06] hover:text-gray-300"}`}
                        >
                          {r}
                        </button>
                      ))}
                ...
                {selectedContinent && (
                  <div className="relative">
                    ...
                    >
                      ...
                      {Object.keys(REGION_DATA[selectedContinent] || {}).map(
                        (country) => (
                          <option
                            key={country}
                            value={country}
                ...
                {selectedCountry &&
                  REGION_DATA[selectedContinent]?.[selectedCountry]?.length >
                    0 && (
                    <div className="relative">
                      <MapPin className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-600 w-4 h-4" />
                      <select
                      ...
                      >
                        ...
                        {REGION_DATA[selectedContinent][selectedCountry].map(
                          (city) => (
                            <option
                              key={city}
                              value={city}
        ...
        {step === 5 && (
          <motion.div variants={itemVariants} className="text-center py-4">
            ...
            <p className="text-gray-400 text-sm mb-6">
              Complete your registration to start your gaming journey on
              Inception Games.
            </p>
            <div className="space-y-3">
              <motion.button
        ...
        {mode === "signup" && step > 1 && step < 5 && (
          <motion.div variants={itemVariants} className="flex gap-2">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className={`flex-1 h-1.5 rounded-full transition ${i <= step ? "bg-purple-500" : "bg-gray-700"}`}
              />
            ))}
          </motion.div>
        )}
