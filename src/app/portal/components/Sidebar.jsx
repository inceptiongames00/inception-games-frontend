      ...
      `}>
        <div className="p-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1 font-medium tracking-wide uppercase">Workspace</p>
          </div>
          <button 
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={`w-5 h-5 transition-transform duration-300 group-hover:scale-110 ${isActive ? "text-purple-600" : "text-gray-400 group-hover:text-gray-600"}`} />
                {item.name}
              </Link>
            );
          })}
